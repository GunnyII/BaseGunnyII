...
namespace Game.Server.GameUtils
{
    public class PlayerEquipInventory : PlayerInventory
    {
        ...
        public PlayerEquipInventory(GamePlayer player)
            : base(player, true, 81, (int)eBageType.MainBag, BAG_START, true)
        {
        }
            ...
            try
            {
                ...
                lock (m_lock)
                {
                    ...
                    for (int i = 0; i < BAG_START; i++)
                    {
                        ...
                        if (i < 5)
                        {
                            ItemInfo c_item = m_player.CardBag.GetItemAt(i);
                           
                            if (c_item != null)
                            {
                                int str = c_item.StrengthenLevel == 0 ? 1 : c_item.StrengthenLevel;
                                attack += c_item.Attack * str;
                                defence += c_item.Defence * str;
                                agility += c_item.Agility * str;
                                lucky += c_item.Luck * str;
                            }
                        }
                    }
                ...
                //m_player.UpdateBaseCardProperties(attack, defence, agility, lucky);
