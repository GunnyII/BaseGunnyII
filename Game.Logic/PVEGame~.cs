...
namespace Game.Logic
{
    public class PVEGame : BaseGame
    {
        ...
        public override bool TakeCard(Player player, int index)
        {
            ...
            SendGamePlayerTakeCard(player, index, medal, gold, money, giftToken); //trminhpc
            return true;
        }
        ...
        */        
        public void GameOver()
        {
            if (GameState == eGameState.Playing)
            {
                ...
                foreach (Player p in players)
                {
                    ...
                    pkg.WriteInt(500);//_loc_7.treatment = _loc_2.readInt();
        ...
        public void GameOverAllSession()
        {
            ...
            if (GameState == eGameState.GameOver)
            {
                ...
                foreach (Player p in players)
                {
                    ...
                    pkg.WriteInt(p.TotalAllScore);//_loc_7.fightGP = _loc_2.readInt();
                    pkg.WriteInt(p.TotalAllCure);//_loc_7.ghostGP = _loc_2.readInt();
                    pkg.WriteInt(500);//_loc_7.gpForVIP = _loc_2.readInt();
                    ...
                    pkg.WriteInt(p.GainGP);//_loc_7.gainGP = _loc_2.readInt()                    
                    pkg.WriteBoolean(IsWin);
                    pkg.WriteInt(m_gameOverResources.Count);
                    foreach (string res in m_gameOverResources)
                    {
            ...
            if (GameState == eGameState.ALLSessionStopped)
            {
                ...
                if (IsWin == true)
                {
                    ...
                    if (GameType == eGameType.Dungeon)
                    {
                        foreach (Player p in players)
                        {
                            p.PlayerDetail.SetPvePermission(m_info.ID, m_hardLevel + 1);
                        }
                    }
                }
        ...
        public void SendMissionInfo()
        {
            ...
            pkg.WriteInt(Param1);
            pkg.WriteInt(Param2);
            pkg.WriteInt(WantTryAgain);//_loc_3.tryagain = event.pkg.readInt();
            SendToAll(pkg);
        }
        ...
        {
            GSPacketIn pkg = new GSPacketIn((byte)ePackageType.GAME_CMD);
            pkg.WriteByte((byte)eTankCmdType.BARRIER_INFO);//GAME_UI_DATA
